...
程序只看**已启用来源中选中的日历**，关键字写在标题任何位置都可以。下表是默认规则，**这些词都可以在 `settings.html` 的「日历标记规则」里改**，也能加新的半开放日（比如「出差」「展会」），并设定哪些档位能看到名字、其他人看到「有安排」还是「不可用」：
...
| `elder` 长辈 | 只显示「空闲 / 忙 / 回老家」，没有时段、活动类型和约饭选项；上班日一律显示忙 | |
| `con` 漫展圈 | 在 xx | 不可用 |
| `close` 密友 | 在 xx | 回老家 |
...
- `until`：失效日期，可不写。
- `forever: true`：永久有效，忽略 `until` 和漫展结束后的自动失效。删掉这一行才会失效。
